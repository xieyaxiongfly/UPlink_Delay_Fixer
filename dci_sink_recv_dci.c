...
int recv_config(char *recvBuf, cell_config_t *cell_config, int buf_idx,
                int recvLen) {
...
void print_ue_dci(ue_dci_t *q) {
  printf("Cell_idx:%d tti:%d rnti:%d dl_tbs:%d ul_tbs:%d\n", q->cell_idx,
         q->tti, q->rnti, q->dl_tbs, q->ul_tbs);
  return;
}
...
int ngscope_dci_sink_recv_buffer(ngscope_dci_sink_CA_t *q, char *recvBuf,
  ...
  if (recvBuf[buf_idx] == (char)0xAA && recvBuf[buf_idx + 1] == (char)0xAA &&
      recvBuf[buf_idx + 2] == (char)0xAA &&
      recvBuf[buf_idx + 3] == (char)0xAA) {
  ...
  } else if (recvBuf[buf_idx] == (char)0xBB &&
             recvBuf[buf_idx + 1] == (char)0xBB &&
             recvBuf[buf_idx + 2] == (char)0xBB &&
             recvBuf[buf_idx + 3] == (char)0xBB) {
  ...
  } else if (recvBuf[buf_idx] == (char)0xFF &&
             recvBuf[buf_idx + 1] == (char)0xFF &&
             recvBuf[buf_idx + 2] == (char)0xFF &&
             recvBuf[buf_idx + 3] == (char)0xFF) {
    printf("EXIT!\n");
    return -999;
  } else {
